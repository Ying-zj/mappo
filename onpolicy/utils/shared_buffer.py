...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        self.episode_length = args.episode_length  # 一个episode的长度
        self.n_rollout_threads = args.n_rollout_threads  # 线程数量
        self.hidden_size = args.hidden_size  # 隐层大小
        self.recurrent_N = args.recurrent_N  # RNN层数
        self.gamma = args.gamma  # discounting factor
        self.gae_lambda = args.gae_lambda  # ？
        self._use_gae = args.use_gae  # 是否使用gae，gae是什么
        self._use_popart = args.use_popart  # 是否使用popart层
        self._use_valuenorm = args.use_valuenorm  # 是否使用归一化
        self._use_proper_time_limits = args.use_proper_time_limits  # 是否使用时间限制，干什么用的？

        obs_shape = get_shape_from_obs_space(obs_space)  # 观测空间大小
        share_obs_shape = get_shape_from_obs_space(cent_obs_space)  # 共享观测空间大小
        ...
        self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *share_obs_shape),
                                  dtype=np.float32)  # 共享观测向量
        self.obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *obs_shape), dtype=np.float32)  # 观测向量
        ...
        self.returns = np.zeros_like(self.value_preds)  # 回报

        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, act_space.n),
                                             dtype=np.float32)  # 可选动作
        else:
            self.available_actions = None

        act_shape = get_shape_from_act_space(act_space)  # 动作空间大小
        ...
        self.rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)  # 奖赏

        self.masks = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.bad_masks = np.ones_like(self.masks)
        self.active_masks = np.ones_like(self.masks)  # mask在哪用？
