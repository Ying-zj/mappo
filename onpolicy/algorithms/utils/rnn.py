...
class RNNLayer(nn.Module):
    def __init__(self, inputs_dim, outputs_dim, recurrent_N, use_orthogonal):
        ...
        self._recurrent_N = recurrent_N  # GRU的层数
        self._use_orthogonal = use_orthogonal  # 是否使用正交初始化

        self.rnn = nn.GRU(inputs_dim, outputs_dim, num_layers=self._recurrent_N)  # 创建GRU层
        for name, param in self.rnn.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0)  # 常数初始化
            elif 'weight' in name:
                if self._use_orthogonal:
                    nn.init.orthogonal_(param)  # 正交初始化
                else:
                    nn.init.xavier_uniform_(param)  # 服从均匀分布的初始化器
        self.norm = nn.LayerNorm(outputs_dim)  # 归一化层
